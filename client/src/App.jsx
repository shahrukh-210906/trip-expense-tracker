...
const pages = { overview: 'Overview', feed: 'Live feed', balances: 'Settlement', purse: 'Kitty history', settings: 'Settings & sync' };
  ...
  return <div className={'mobile-app '+(!session?'auth-shell':'')}>
    ...
    <main className="mobile-main">
      ...
      {!session||!tripId?<><Onboarding session={session} onSession={setSession} onJoinSuccess={selectTrip} trips={trips}/>{session&&<InstallApp/>}</>:!data?<div className="loading-state"><Icon name="trip" size={32}/><h1>{error?'Trip unavailable':'Loading your trip…'}</h1><button className="secondary" onClick={backToTrips}>Back to my trips</button></div>:<>
        <div className="mobile-page-heading"><span className="eyebrow">{page==='overview'?'TRIP OVERVIEW':'TRIP DETAILS'}</span><h1>{page==='overview'?data.trip.name:pages[page]}</h1></div>
        ...
        {page==='settings'&&<Settings session={session} disconnected={disconnected} status={status} queued={queued} busy={refreshing||offline.busy} onSync={()=>{offline.sync();void refresh();}} onOpenTrip={id=>{selectTrip(id);setPage('balances');}} onLogout={logout}/> }{page==='feed'&&<LiveFeed data={data} rows={queued} name={name} userId={userId}/>}
      ...
      {session&&!tripId&&<><NotificationInbox key={session.user._id} session={session} onOpenTrip={id=>{selectTrip(id);setPage('balances');}}/><PaymentProfile key={session.user._id}/></>}
      <Tutorial/>
    </main>
    {session&&tripId&&data&&<>{data.trip.status!=='ended'&&page!=='settings'&&<button aria-label="Add expense" className="expense-fab" onClick={()=>setEntryKind('personal')}><Icon name="plus"/><span>Add expense</span></button>}<nav className="bottom-nav" aria-label="Trip navigation">{[['overview','Overview','trip'],['feed','Expenses','expenses'],['balances','Balances','balances'],['settings','Settings','settings']].map(([id,label,icon])=><button key={id} aria-current={(page===id||(id==='overview'&&page==='purse'))?'page':undefined} onClick={()=>{setPage(id);window.scrollTo({top:0,behavior:'instant'});}}><Icon name={icon}/><span>{label}</span></button>)}</nav></>}
