...
const noop=()=>{};
function Capture(){
 const [screen,setScreen]=useState(new URLSearchParams(location.search).get('screen')||'trips');
 ...
 {screen==='trips'||screen==='install'?<><Onboarding session={{user:members[0]}} trips={[trip]} onSession={noop} onJoinSuccess={()=>setScreen('overview')}/>{screen==='install'&&<InstallApp/>}</>:<><div className="mobile-page-heading"><span className="eyebrow">TRIP DETAILS</span><h1>{screen==='settlement'?'Settlement':trip.name}</h1></div>{screen==='settlement'?<Balances data={data} userId="demo-a" name={name}/>:<Overview data={data} userId="demo-a" name={name} lead manage={()=>setScreen('kitty')} history={noop} feed={noop}/>}</>}
 </main>{!['trips','install'].includes(screen)&&<><button aria-label="Add expense" className="expense-fab" onClick={()=>setScreen('expense')}><Icon name="plus"/><span>Add expense</span></button><nav className="bottom-nav">{[['trip','Overview','overview'],['expenses','Expenses','overview'],['balances','Balances','settlement'],['settings','Settings','install']].map(([icon,label,id])=><button key={label} onClick={()=>setScreen(id)} aria-current={id===(screen==='settlement'?'settlement':'overview')&&label!=='Expenses'?'page':undefined}><Icon name={icon}/><span>{label}</span></button>)}</nav></>}{expense&&<ExpenseDialog initialKind={screen==='kitty'?'contribution':'personal'} data={data} user={members[0]} onClose={()=>setScreen('overview')} onSaved={()=>setScreen('overview')} enqueue={noop}/>}</div>;
}
